package com.bsva.dcms.commons.views.temp;
...
public class CsvCcr00XDataView {
	...
	private List<CsvCcr00XDataViewDto> csvCcr00XDataViewDtoList = new ArrayList<>();;
	private List<ReportLayoutDto> reportLayoutDtos = new ArrayList<>();;
	...
	public List<CsvCcr00XDataViewDto> execute(){
		...
		List<ReportLayoutDto> reportLayoutDtos =  buildReportLayout(); //member X member X service X subservice X tran type X card type
		Map<String,ReportData> reportDataGroup = buildReportData(); //sum tran amount, billing fee, billing fee amount, billing vat - including cashbacks by key
		
		/*
		 * get all the bank X bank X tran code X card type and assign the sums
		 */
		for(ReportLayoutDto reportLayoutDto : reportLayoutDtos){
			...
			for(String key : keys){
				...
				if (key.startsWith(key2)){  // this is because key2 does not contain destination report and amount direction
					...
					csvCcr00XDataViewDto.setTransactionValue((long)reportDataForKey.getTransactionAmount());
					csvCcr00XDataViewDto.setBillingFee(new BigDecimal(reportDataForKey.getBillingFee()).setScale(2).doubleValue());
...
//			isMemberValid = false;
	...
	private Map<String,ReportData> buildReportData(){
		...
		/*
		 * take all the transactions you got and group them by acq|iss|service|card type|tran code|dest report|amount|dir e.g lets say you received 4 of those
		 * then sum their tran amount, billing fee, billing fee amount, billing vat - including cashbacks
		 * group them into 1 item
		 * also determine the actual issuer and acquirer based on dest_report : did the bank send this to us as an issuer or an acquirer
		 */
