...
public class CRFileWriter {
    ...
    public CRFileWriter(String subServiceID,
                      String reportFolder,
                      String reportId) {
    ...
    protected String csr00xFilename(Integer issuerMemberCode, String filename) {
    	 ...
    	 String prepareFileName = filename.substring(2);
        return reportFolder +
                FILE_SEPARATOR +
                "CR" +
                reportId +
                SUB_SERVICE_MNEMONICS.get(subServiceID) +  "." +
                "CR"+prepareFileName.trim() + "." +
     ...
     * @param out
     */
    protected void writeControlHeaderData(FileHeaderDTO header, PrintWriter out) {
        writeControlHeaderData(null, header, out);
    }
     ...
     * @param out
     */
    protected void writeControlHeaderData(Date processDate, FileHeaderDTO header, PrintWriter out) {

        Date date = header.getFileDate() != null ? header.getFileDate() : processDate;
        String formattedDate = FULL_DATE_FORMAT.format( date );
        String fileNamePrefix = header.getFilename().substring(2);
        String seqNum = header.getFilename().substring(4, 7);
        ...
        String s =
                ...
                format("CR"+fileNamePrefix,                   8, ' ', Justification.LEFT) +
                format( seqNum,             4, '0', Justification.RIGHT) +
     ...
     * @param out
     */
    protected void writeFileHeaderData( CompanyParametersEntity params,
    ...
    protected void writeControlTrailerData(FileHeaderDTO header, PrintWriter out) {
        writeControlTrailerData(null, header, out);
    }
    /**
     ...
     * @param out
     */
    protected void writeControlTrailerData(Date processDate, FileHeaderDTO header, PrintWriter out) {
    ...
    protected FileHeaderDTO defaultFileHeaderData() {
