...
public class CCR15Reports {
	...
	private static String reportId = "015";
   ...
   // private static private BigInteger countNum = BigInteger.ONE;
	...
	public Set<Integer> filterListData(List<Css_Stats_View> listData){
		...
		for (Css_Stats_View css_Stats_View : listData) {
			dataset.add(Integer.valueOf(css_Stats_View.getCss_Stats_View_PK().getAcquiringMember()));
		}
		return dataset;
	}
	...
	public void writeCsvFile(String subService) {
		...
		FileWriter fileWriter = null;
			List<Css_Stats_View> listData = new Css_Stats_View_DAO().getCSS0015Reports(subService);
			Set<Integer> datasetList = filterListData(listData);
			for (Integer acquiringNumber : datasetList) {
			try {
			CSFMemberServiceDTO codeDetails = getMemberNo(Integer.valueOf(acquiringNumber),subService);
			if(codeDetails == null){
				continue;
			}
			// create output file
			filename = ccr00xFilename(subService,
			...
			header.setOutputDate(procdate);
			header.setService("CARD");
			header.setSubService("REP");
			header.setBankMemberNumber("");
			header.setBankMemberNumber(padLeftString(String.valueOf(acquiringNumber), 4));
			header.setOriginator("ACBJ");
			header.setFileName("");
			header.setDataType("DATA");
			header.setDataDirection("OUT");
			header.setSettlementDate(procdate);
			header.setTestLiveInd("LIVE");
			header.setNumber("01320");
			header.setBankCode(padLeftString(String.valueOf(acquiringNumber), 3));
			header.setReportName("CCR015");
			...
			for (Css_Stats_View css_Stats_View : listData) {
				...
				ccr014ReportsDto.setAcquiringMember(String.valueOf(css_Stats_View.getCss_Stats_View_PK().getAcquiringMemberName()));
				ccr014ReportsDto.setCardDescription(css_Stats_View.getCss_Stats_View_PK().getCardDescription());
				ccr014ReportsDto.setInterchangeRateDesignator(String.valueOf(css_Stats_View.getInterchangeRateDesignator()));
				ccr014ReportsDto.setIssuerBin(css_Stats_View.getCss_Stats_View_PK().getIssuerBin());
				ccr014ReportsDto.setIssuingMember(String.valueOf(css_Stats_View.getCss_Stats_View_PK().getIssuingMemberName()));
				ccr014ReportsDto.setItemCharge(String.valueOf(css_Stats_View.getCss_Stats_View_PK().getItemCharge()));
				ccr014ReportsDto.setItemChargeAmount(String.valueOf(css_Stats_View.getCss_Stats_View_PK().getItemChargeAmount()));
			...
			fileFooterDto.setOutputDate(procdate);
			fileFooterDto.setServiceCode(String.valueOf(acquiringNumber));
			...
			fileFooterDto.setSubService("REP");
	...
	public static void main(String[] args){
		new CCR15Reports().writeCsvFile("FLEET CARD");
	}
